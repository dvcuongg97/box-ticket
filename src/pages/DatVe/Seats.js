...
import "../../asset/DatVe/SeatStyle.css";
import { useDispatch, useSelector } from "react-redux";
import { setArrGheDangDat } from "../../redux/datVeSlice";
...
export default function Seats() {
  ...
  const { userProfile } = useSelector((state) => state.userInfoSlice);
  const { dataPhongVe, arrGheDangDat } = useSelector(
    (state) => state.datVeSlice
  );
  const { danhSachGhe, thongTinPhim } = dataPhongVe;
  ...
  let renderSeats = () => {
    return danhSachGhe?.map((ghe, index) => {
      ...
      let classGheUserDat = "";
      if (userProfile?.taiKhoan === ghe.taiKhoanNguoiDat) {
